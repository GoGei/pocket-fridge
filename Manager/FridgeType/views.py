...
def fridge_type_add(request):
    ...
    if form_body.is_valid():
        ...
        messages.success(request, _(f'Fridge type {fridge_type.label} was successfully created'))
        return redirect(reverse('manager-fridge-type-list', host='manager'))
...
def fridge_type_edit(request, fridge_type_id):
    ...
    if form_body.is_valid():
        fridge_type = form_body.save()
        messages.success(request, _(f'Fridge type {fridge_type.label} was successfully edited'))
        return redirect(reverse('manager-fridge-type-view', args=[fridge_type_id], host='manager'))
...
def fridge_type_archive(request, fridge_type_id):
    ...
    if categories.exists():
        messages.warning(request, ungettext_lazy('This fridge type is used in fridge %s time!',
                                                 'This fridge type is used in fridges %s times!') % categories.count())
    else:
        fridge_type.archive(request.user)
        messages.success(request, _(f'Fridge type {fridge_type.label} was successfully archived'))
    return redirect(reverse('manager-fridge-type-list', host='manager'))
...
def fridge_type_restore(request, fridge_type_id):
    ...
    messages.success(request, _(f'Fridge type {fridge_type.label} was successfully restored'))
    return redirect(reverse('manager-fridge-type-list', host='manager'))
...
def fridge_type_load_fixture(request):
    ...
    if form_body.is_valid():
        try:
            ...
            messages.success(request, _(f'Fridge type {created_count} was successfully created'))
            messages.success(request, _(f'Fridge type {len(items)} was successfully updated'))
            return redirect(reverse('manager-fridge-type-list', host='manager'))
...
def fridge_type_load_default_fixture(request):
    ...
    messages.success(request, _('Fridge type default fixture was successfully loaded'))
    return redirect(reverse('manager-fridge-type-list', host='manager'))
